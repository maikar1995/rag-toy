#!/usr/bin/env python3
"""
Standalone PDF Parser - Extracted from RAGFlow
A lightweight, self-contained PDF parser that extracts text and basic structure from PDFs.
Can be used independently in any project without RAGFlow's architecture dependencies.

Usage:
    from standalone_pdf_parser import StandalonePDFParser

    parser = StandalonePDFParser()
...
# Add project root to path to import standalone parser
...
def demo_usage():
    """Demonstration of how to use the standalone parser."""

    # Initialize parser
    parser = StandalonePDFParser(
        enable_ocr=True, ocr_lang="eng", min_text_height=8, merge_threshold=1.5
    )

    # Example usage
    pdf_file = "data/input/_OceanofPDF.com_The_Minto_Pyramid_Principle_-_Barbara_Minto.pdf"  # Replace with your PDF

    try:
        # Parse entire document
        result = parser.parse(pdf_file)

        print("=== PDF Parser Results ===")
        ...
        # Print first page as sample
        if result["pages"]:
            print("\n=== First Page Sample ===")
            print(
                result["pages"][0][:500] + "..."
                if len(result["pages"][0]) > 500
                else result["pages"][0]
            )

        # Parse specific page range
        result_partial = parser.parse(pdf_file, page_range=(0, 2))
        print("\n=== Pages 1-2 only ===")
        print(f"Pages extracted: {len(result_partial['pages'])}")
...
def main():
    ...
    input_group.add_argument(
        "--input_dir", type=str, help="Directory containing PDF files"
    )
    input_group.add_argument(
        "--single_file", type=str, help="Single PDF file to process"
    )
    ...
    # Find PDF files
    input_path = args.input_dir or args.single_file
